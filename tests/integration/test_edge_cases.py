...
def iter_group(group_name):
    return SUITE["groups"][group_name]
...
@pytest.mark.p0
@pytest.mark.parametrize("case", iter_group("amplifier_stacking"), ids=lambda c: c["id"])
...
@pytest.mark.p0
@pytest.mark.parametrize("case", iter_group("embedding_fallback"), ids=lambda c: c["id"])
...
def test_snapshot_restore(case):
    # Here you would call your real snapshot restore API
...
def test_telemetry(session, telemetry, case):
    # Simulate emitting metrics based on case
    if case["id"] == "TC-INT-050":
        telemetry.emit("tag_switch_rate", 0.5)
        value = telemetry.metrics["tag_switch_rate"]
        assert 0.0 <= value <= 1.0
    if case["id"] == "TC-INT-051":
        telemetry.emit("amplifier_activation_rate", 0.4)
        assert abs(telemetry.metrics["amplifier_activation_rate"] - 0.4) < 0.01
    if case["id"] == "TC-INT-052":
        telemetry.emit("prompt_injection_count", 2)
        assert telemetry.metrics["prompt_injection_count"] == 2
    if case["id"] == "TC-INT-053":
        telemetry.emit("tag_switch_rate", 0.0)
        telemetry.emit("amplifier_activation_rate", 0.0)
        telemetry.emit("prompt_injection_count", 0)
        for m in ["tag_switch_rate", "amplifier_activation_rate", "prompt_injection_count"]:
